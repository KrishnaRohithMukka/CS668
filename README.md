...
* **Results:** In this project we successfully implemented a anomaly detection system using the YOLOv8n model, trained on a dataset of five critical anomaly types: violence, vandalism, fire, abuse, and accidents.
The uploaded video is analyzed frame by frame and look for anomalies and detects it with higher accuracy.
The Evaluation Metrics: Precision : 1.00, Recall : 0.80, F1 Score: 0.89, Mean IoU: 0.70
Frames containing detected anomalies were saved, annotated with bounding boxes and class labels.
The notification system successfully sends the user notification immediately after detecting the anomalies.
